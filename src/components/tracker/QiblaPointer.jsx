...
export default function QiblaPointer() {
  ...
  useEffect(() => {
    if (location && location.type === 'coords' && location.lat && location.lon) {
      ...
      setQiblaAngle(null); // Wait for location
    }
  }, [location]);
  ...
  const isPointed = qiblaAngle !== null && Math.abs((heading - qiblaAngle + 360) % 360) < 5 || Math.abs((heading - qiblaAngle + 360) % 360) > 355;
    ...
    <div className="rounded-2xl border border-border bg-card p-6 flex flex-col items-center text-center relative overflow-hidden">
      ...
      <div className="relative w-48 h-48 mb-6">
        ...
        >
          ...
          {/* Qibla Indicator Arrow (Relative to North) */}
          {qiblaAngle !== null && (
            <div 
            ...
            >
              <div className="absolute top-0 left-1/2 -translate-x-1/2 -translate-y-1 flex flex-col items-center">
                <Navigation2 className={`w-8 h-8 transition-colors duration-500 ${isPointed ? 'text-primary fill-primary drop-shadow-[0_0_12px_rgba(212,175,55,1)]' : 'text-primary/70 fill-primary/30'}`} />
                {/* Crescent and Star Indicator */}
        ...
        {/* Center Kaaba Motif (Static, doesn't rotate) */}
